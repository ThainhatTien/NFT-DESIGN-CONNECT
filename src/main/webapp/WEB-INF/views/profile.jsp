			...
			<div class="page-heading">
				<div class="media clearfix">
					...
					<div class="media-body va-m">
						...
						<%-- Kiểm tra xem người dùng có quyền thiết kế không --%>
						<c:if test="${not empty user && user.role eq 'DESIGN'}">
							<a class="btn btn-success" href="/Specifications">Thêm bản vẽ</a>
						</c:if>
						<div id="connectMessage" style="display: none;">Connected to
							Phantom Wallet successfully!</div>
        ...
        document.getElementById('connectButton').addEventListener('click', async () => {
            if (window.solana && window.solana.isPhantom) {
                try {
                    ...
                    // Gửi thông tin public key và userId đến server Java
                    fetch('/connect', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                        body: JSON.stringify({ publicKey: response.publicKey.toString(), userId: ${user.user_id} })
                      ...
                      .then(data => {
                          if (data.status === 'success') {
                              ...
                              console.error('Error:', data.message); // Xử lý lỗi nếu có
                          }
                      })
